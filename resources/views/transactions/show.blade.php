            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-8 p-4 border border-gray-300 dark:border-gray-700 rounded-md">
                        ...
                        </div> {{-- Penutup div mb-8 p-4 border --}}
                    </div> {{-- Penutup div mb-8 p-4 border --}}
                    ...
                    <div class="mb-8 p-4 border border-gray-300 dark:border-gray-700 rounded-md">
                        ...
                        @if ($transaction->invoice)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    <p class="text-gray-600 dark:text-gray-400"><strong>File PO:</strong> <a href="{{ Storage::url(str_replace('public/', 'storage/', $transaction->invoice->po_file)) }}" target="_blank" class="text-blue-500 hover:underline">Lihat File PO</a></p>
                                </div>
                                @endif

                                {{-- *** Tambahan untuk Detail Pembayaran Invoice *** --}}
                                @if ($transaction->invoice->payment_received_date)
                                    <div class="md:col-span-2 mt-4">
                                        <h5 class="font-semibold text-gray-700 dark:text-gray-300 mb-2">Detail Pembayaran Diterima:</h5>
                                        <p class="text-gray-600 dark:text-gray-400"><strong>Tanggal Pembayaran Diterima:</strong> {{ \Carbon\Carbon::parse($transaction->invoice->payment_received_date)->translatedFormat('d M Y') }}</p>
                                        <p class="text-gray-600 dark:text-gray-400"><strong>Metode Pembayaran:</strong> {{ $transaction->invoice->payment_method ?? '-' }}</p>
                                        @if ($transaction->invoice->payment_proof_file)
                                            <p class="text-gray-600 dark:text-gray-400"><strong>Bukti Pembayaran:</strong>
                                                <a href="{{ Storage::url(str_replace('public/', 'storage/', $transaction->invoice->payment_proof_file)) }}" target="_blank" class="text-blue-500 hover:underline">Lihat Bukti Pembayaran</a>
                                            </p>
                                        @else
                                            <p class="text-gray-600 dark:text-gray-400"><strong>Bukti Pembayaran:</strong> Tidak ada.</p>
                                        @endif
                                        <p class="text-gray-600 dark:text-gray-400"><strong>Reminder Terakhir Dikirim:</strong> {{ $transaction->invoice->reminder_sent_at ? \Carbon\Carbon::parse($transaction->invoice->reminder_sent_at)->translatedFormat('d M Y H:i') : '-' }}</p>
                                    </div>
                                @endif
                                {{-- *** Akhir Tambahan Detail Pembayaran Invoice *** --}}
                            </div>
                        @else
                ...
                <div class="mt-6 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-4 p-6"> {{-- Tambahkan padding di sini --}}
