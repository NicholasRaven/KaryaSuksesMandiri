            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('users.update', $customer->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="type" class="block font-medium text-sm text-gray-700 dark:text-gray-300">User Role:</label>
                            <select name="type" id="type" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm block mt-1 w-full @error('type') border-red-500 @enderror" required>
                                ...
                                <option value="Pimpinan" {{ old('type', $user->user) == 'Pimpinan' ? 'selected' : '' }}>Pimpinan</option>
                                <option value="Admin" {{ old('type', $user->type) == 'Admin' ? 'selected' : '' }}>Admin</option>
                                <option value="Marketing" {{ old('type', $user->type) == 'Marketing' ? 'selected' : '' }}>Marketing</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
